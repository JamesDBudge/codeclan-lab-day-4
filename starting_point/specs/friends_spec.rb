...
class TestFriends < MiniTest::Test
  ...
  def setup #using this allows every test to have a clean slate
...
def test_add_friend
  add_friend("Scrappy-Doo", @person2)
  result = @person2[:friends].include?("Scrappy-Doo")
  assert_equal(true, result)
end
...
def test_remove_friend
  ...
  result = @person1[:friends].include?("Velma")
...
def test_loan
  ...
  result = loan(@person5, @person4, 1000)

  assert_equal(-900, @person5[:monies])
  assert_equal(1020, @person4[:monies])
end
...
def test_all_favourite_foods
  ...
  assert_equal(["charcuterie", "soup", "bread", "Scooby snacks", "spaghetti", "ratatouille", "spinach"].sort , result)
end
